...
namespace Timer {
  ...
  void begin(int interruptPin, int mode)
  {
    ...
    goodResponse = goodResponse && (endTransmission == 0); 
  ...
  void startTimer(int timeSetting)
  {
    ...
    if (!configured)
      Serial.println(F("RTC no worky"));
      // Error::raiseError(F(__FILE__), NULL, F("RTC not configured")); // For .h file
    ...
    if (maxCount <= 0)
      Error::raiseError(F(__FILE__), NULL, F("Bad timer value")); // For .h file

    if (maxCount <= 100)
      Error::raiseWarning(F(__FILE__), NULL, F("Low timer value detected")); // For .h file
