...
namespace OLED2
{
    ...
    bool begin()
    {
        ...
        if (!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS))
            Error::raiseError(__FILE__, __func__, "OLED 2 allocation failed");
    ...
    void showVolumeLevel(int vol, int color)
    {
		display.fillCircle(vol, CIRCLE_Y, CIRCLE_R, color);
		display.display();
    }

    void clearVolumeLevel(int vol)
    {
        showVolumeLevel(vol, BLACK);
    }
